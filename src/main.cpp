...
int nCoinbaseMaturity = COINBASE_MATURITY_SHND;
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (undo.nHeight != 0) {
                    ...
                    coins.fCoinStake = undo.fCoinStake; // stronghands
                    coins.nTime = undo.nTime;           // stronghands
                } else {
                    if (coins.IsPruned())
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // stronghands: coinbase reward check relocated to CheckBlock()
...
bool InitBlockIndex() {
    ...
    fTxIndex = true; // stronghands: txindex is always enabled
    ...
    if (!fReindex) {
        ...
        // stronghands: check genesis block
        {
            CValidationState state;
...
string GetWarnings(string strFor)
{
    ...
    // stronghands: checkpoint warning
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            // stronghands: relay memory may contain blocks too
            bool found = false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("StronghandsMiner : proof-of-work not meeting target");

    //// debug print
    printf("StronghandsMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StronghandsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StronghandsMiner : ProcessBlock, block not accepted");
    }
...
#endif
{
    ...
    RenameThread(fProofOfStake? "stronghands-stake-minter" : "stronghands-miner");
    ...
    try { loop {
        ...
        printf("Running StronghandsMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("StronghandsMiner terminated\n");
        throw;
    }
}
