...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Stronghands version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "stronghands:") && !boost::algorithm::istarts_with(argv[i], "stronghands:"))
                fCommandLine = true;
...
{
    RenameThread("stronghands-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    //
    // stronghands: -mintxfee and -minrelaytxfee options of bitcoin disabled
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Stronghands is probably already running."), strDataDir.c_str()));
