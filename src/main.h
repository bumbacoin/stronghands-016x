...
public:
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // stronghands: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // stronghands: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // stronghands: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // stronghands: whether transaction is a coinstake
    bool fCoinStake;

    // stronghands: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // stronghands: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // stronghands
            undo.nTime = nTime;            // stronghands
        }
        return true;
    }
...
public:
    ...
    // (memory only) Total amount of trust score (stronghands proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // stronghands: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // stronghands: proof-of-stake related block index fields
