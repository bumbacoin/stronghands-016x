...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Stronghands
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Stronghands
    // Mac: ~/Library/Application Support/Stronghands
    // Unix: ~/.stronghands
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Stronghands";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "stronghands.conf"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir(false) / pathConfigFile;

    // Load old config file if present
    if (mapArgs.count("-conf") == 0 && !boost::filesystem::exists(pathConfigFile)) {
        boost::filesystem::path pathOldConfigFile = GetDataDir(false) / "stronghands.conf";
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "stronghandsd.pid"));
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "Stronghands:" << FormatVersion(PEERCOIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
