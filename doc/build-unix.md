...
---------------------
	...
	mkdir -p ~/.stronghands
	echo "rpcuser=username" >>~/.stronghands/stronghands.conf
	echo "rpcpassword=`head -c 32 /dev/urandom | base64`" >>~/.stronghands/stronghands.conf
...
Note: Currently, there is no stronghands ebuild available in overlay 
...
Notes
-----
The release is built with GCC and then "strip stronghandsd" to strip the debug
